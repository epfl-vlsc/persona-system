...
class AgdImportSraOp : public OpKernel {
 ...
 public:
  ...
  explicit AgdImportSraOp(OpKernelConstruction* ctx) : OpKernel(ctx) {
    ...
    iterator =
        new ReadIterator(reader.getReadRange(start, count));  // 1 numReads
  }

  Status GetOutputBufferPair(OpKernelContext* ctx,
                             ResourceContainer<BufferPair>** ctr) {
  ...
  Status InitHandles(OpKernelContext* ctx) {
    TF_RETURN_IF_ERROR(
        GetResourceFromContext(ctx, "bufpair_pool", &bufpair_pool_));
  ...
  Status GetBufferForBuilder(OpKernelContext* ctx, ColumnBuilder& builder,
                             Tensor* out, int index) {
  ...
  void Compute(OpKernelContext* ctx) override {
    ...
    OP_REQUIRES_OK(
        ctx, ctx->allocate_output("chunk_out", TensorShape({3, 2}), &out_t));
    OP_REQUIRES_OK(
        ctx, ctx->allocate_output("num_records", TensorShape({}), &num_recs_t));
    OP_REQUIRES_OK(ctx, ctx->allocate_output("first_ordinal", TensorShape({}),
                                             &first_ord_t));
 ...
 private:
  ...
  Status Process(ColumnBuilder& bases, ColumnBuilder& qual, ColumnBuilder& meta,
                 int& num_recs) {
    num_recs = 0;
    for (size_t i = 0; i < chunk_size_; i++) {
      ...
      TF_RETURN_IF_ERROR(IntoBases(iterator->getReadBases().data(),
                                   iterator->getReadBases().size(), bases_));
      bases.AppendRecord(reinterpret_cast<const char*>(&bases_[0]),
                         sizeof(BinaryBases) * bases_.size());
      qual.AppendRecord(iterator->getReadQualities().data(),
                        iterator->getReadQualities().size());
      meta.AppendRecord(iterator->getReadName().data(),
                        iterator->getReadName().size());
      num_recs++;
    }
    return Status::OK();
  }
...
REGISTER_KERNEL_BUILDER(Name("AgdImportSra").Device(DEVICE_CPU),
                        AgdImportSraOp);
