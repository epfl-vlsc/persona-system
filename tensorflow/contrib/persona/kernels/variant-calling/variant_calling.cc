...
namespace tensorflow {
  ...
  class VariantCallingOP : public OpKernel {
  public:
    VariantCallingOP(OpKernelConstruction *context) : OpKernel(context) {
      ...
      OP_REQUIRES_OK(context, context->GetAttr("num_datasets", &vec_size));
      OP_REQUIRES_OK(context, context->GetAttr("cmd_line", &command_line_));
	...
	cout << "zfs";
      //cout << "started" <<endl;
      AGDMultiReader multiReader(ctx,vec_size);

      int cx = 0;
      //BamTools :: BamAlignment next;
      // int last_index = -1;
      // int last_position = -1;
      // while(multiReader.getNextAlignment(ctx,next)){
      //   cx++;
      // }
      cout << cx;
      signal(SIGSEGV, segfaultHandler);

      int argc = command_line_.size()+1;
      char** argv = new char*[argc];
      string freebayes = "freebayes";
      argv[0] = new char[freebayes.size() + 1];
      for(int j=0;j<freebayes.size();j++)
      {
        argv[0][j] = freebayes.at(j);
      }
      argv[0][freebayes.size()] = '\0';
      //std::strcpy(argv[0], freebayes.c_str());
      for(size_t i = 1; i < argc; ++i)
      {
          argv[i] = new char[command_line_[i-1].size() + 1];
          for(int j=0;j<command_line_[i-1].size();j++)
          {
            argv[i][j] = command_line_[i-1].at(j);
          }
          argv[i][command_line_[i-1].size()] = '\0';
          //std::strcpy(argv[i], command_line_[i].c_str());
      }


      AlleleParser* parser = new AlleleParser(argc,argv,multiReader,ref_seqs_,ref_sizes_);
      ...
      Tensor *out_t;
      OP_REQUIRES_OK(ctx, ctx->allocate_output(0, TensorShape({4, 2}), &out_t));
  ...
  REGISTER_KERNEL_BUILDER(Name("VariantCalling").Device(DEVICE_CPU), VariantCallingOP);
} //  namespace tensorflow {
