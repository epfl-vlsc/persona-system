// Modified version of kernels/sort/agd_sort_op.cc to sort proteins based on their sequence length
// Milad, Aug. 2018
...
namespace tensorflow {
    ...
    class AGDProteinSortOp : public OpKernel {
    public:
        AGDProteinSortOp(OpKernelConstruction *context) : OpKernel(context) {
            OP_REQUIRES_OK(context, context->GetAttr("sort_ascending", &sort_ascending_));
        }
        ...
        void Compute(OpKernelContext* ctx) override {
            ...
            vector<AGDRecordReader> proteins_vec;
            OP_REQUIRES_OK(ctx, LoadDataResources(ctx, results_in, proteins_vec, num_records_t, releasers));
            ...
            for (int i = 0; i < num_results; i++) {
                auto& protein_reader = proteins_vec[i];
                status = protein_reader.GetNextRecord(&data, &size);
                // go thru the results, build up vector of location, index, chunk
                int j = 0;
                while(status.ok()) {
                    entry.proteinSize = size;
                    entry.chunk = i;
                    entry.index = j;
            ...
            //bool sort_ascending = sort_ascending_;
            std::sort(sort_index_.begin(), sort_index_.end(), [this](const SortEntry& a, const SortEntry& b) {
                if (this->sort_ascending_)
                    return a.proteinSize < b.proteinSize;
                return a.proteinSize > b.proteinSize;
            ...
            for (size_t i = 0; i < sort_index_.size(); i++) {
                ...
                auto& protein_reader = proteins_vec[entry.chunk];
                protein_reader.GetRecordAt(entry.index, &data, &size);
...
//      LOG(INFO) << "DONE running sort!!";
    ...
    REGISTER_KERNEL_BUILDER(Name("AGDProteinSort").Device(DEVICE_CPU), AGDProteinSortOp);
} //  namespace tensorflow {
