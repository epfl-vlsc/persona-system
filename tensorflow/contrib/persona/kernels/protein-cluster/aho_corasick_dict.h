...
class AhoCorasickDict {
 ...
 public:
  ...
  void AddRepresentative(const std::string& rep) {
    BuildOrUpdateKmerSet(&kmer_set_, rep.c_str(), rep.size());  // update set with rep kmers
  ...
  float Jaccard(const char* query, size_t len) {
    ...
    //auto total_kmers_query = query_set.size();
    auto total_kmers = kmer_set_.size();

    float jaccard = float(results.size()) /
                    float(total_kmers + len - results.size());
    return jaccard;
  }
