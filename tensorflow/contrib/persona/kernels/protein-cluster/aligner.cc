...
// #include
// "tensorflow/contrib/persona/kernels/protein-cluster/minhash/minhash_distance.h"
...
Status ProteinAligner::AlignSingle(const char* seq1, const char* seq2,
                                   int seq1_len, int seq2_len,
                                   Alignment& result) {
  Status s;
...
Status ProteinAligner::AlignLocal(const char* seq1, const char* seq2,
                                  int seq1_len, int seq2_len,
                                  Alignment& result) {
  // find starting point
...
Status ProteinAligner::AlignDouble(const char* seq1, const char* seq2,
                                   int seq1_len, int seq2_len,
                                   bool stop_at_threshold, Alignment& result,
...
bool ProteinAligner::minhash_PassesThreshold(const char* seq1, const char* seq2,
                                             int seq1_len, int seq2_len) {
  ...
  setAlphabetFromString(
      parameters, alphabetProtein);  // alphabetProtein declared in sketch.h

  mash::minhash_distance minhash;
  mash::minhash_distance::CompareOutput* distances =
      minhash.run(seqref, seqqry, seq1_len, seq2_len, parameters);

  // there should be only one pair, as we are passing only one pair to run()
  // command.

  const mash::minhash_distance::CompareOutput::PairOutput* pair =
      &(distances->pairs[0]);
...
bool ProteinAligner::minhash_PassesThreshold_seqsketch(Sketch data_sketch,
                                                       const char* seq2,
                                                       int seq1_len,
                                                       int seq2_len) {
  char* seqqry = denormalize(seq2, seq2_len);
  ...
  mash::minhash_distance::CompareOutput* distances = minhash.run_seqsketch(
      data_sketch, seqqry, seq1_len, seq2_len, parameters);
  ...
  double score = distance * 100;
  if (score == 0) {
    cout << " in passesThreshold_seqsktech score is " << score
         << " , and seq1_len is " << seq1_len << " seq2_len is " << seq2_len
         << " Sketch length is " << data_sketch.getMinHashesPerWindow()
         << " Kmer size is " << data_sketch.getKmerSize() << endl;
...
bool ProteinAligner::minhash_PassesThreshold_seqsketch_repsketch(
  ...
  mash::minhash_distance::CompareOutput* distances =
      minhash.run_seqsketch_repsketch(data_sketch, ref_sketch, seq1_len,
                                      seq2_len, parameters);
  ...
  if (score == 0) {
    cout << " in passesThreshold_seqsktech score is " << score
         << " , and seq1_len is " << seq1_len << " seq2_len is " << seq2_len
         << " Sketch length is " << data_sketch.getReferenceCount() << " and "
         << ref_sketch.getReferenceCount() << " Kmer size is "
         << data_sketch.getKmerSize() << " and  " << ref_sketch.getKmerSize()
         << endl;
    // cout<< "Printing the protein in the == zero case " << endl;
    // for (size_t i = 0; i < seq2_len; i++) {
    //     cout << seqqry[i];
  ...
  // LOG(INFO) << "score is in seqsketch_repsketch " << score <<" MINHASH
  // NUMBERS"; cout << " in passesThreshold_seqsktech score is " << score <<" ,
  // and seq1_len is " << seq1_len << " seq2_len is "<< seq2_len << " Sketch
  // length is " <<   data_sketch.getReferenceCount() << " and " <<
  // ref_sketch.getReferenceCount() << " Kmer size is "
