...
namespace tensorflow {
  ...
  bool Cluster::EvaluateSequence(Sequence& sequence,  
    ...
    for (size_t i = 0; i < params->max_representatives && i < seqs_.size(); i++) {
      ...
      SSWAlignmentResult alignmentResult = aligner.PassesThresholdSSW(sequence.data, rep.Data(), sequence.length, rep.Length()); // milad
...
//          AddCoveredRange(*sequence.coverages, alignmentResult.ref_begin1, alignmentResult.ref_end1);
  ...
  Status Cluster::BuildOutput(vector<Tensor>& match_ints, 
    ...
    //    string PrintDebuggingStuff(bool isIf, SSWAlignmentResult alignmentResult, ProteinAligner::Alignment alignment) {
//        string retVal = "else:\n";
//        if (isIf)
//            retVal = "if:\n";
//        retVal += string("alignmentResult:\n") + "ref:\t" + to_string(alignmentResult.ref_begin1) + "\t" + to_string(alignmentResult.ref_end1) + "\n";
//        retVal += string("read\t") + to_string(alignmentResult.read_begin1) + "\t" + to_string(alignmentResult.read_end1) + "\n";
//        retVal += string("//\n") + "alignment:\n";
