...
class GenomeIndexOp : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* ctx) override {
    ...
    if (!genome_handle_set_) {
      OP_REQUIRES_OK(ctx, SetGenomeHandle(ctx));
    }
 ...
 private:
  Status SetGenomeHandle(OpKernelContext* ctx) EXCLUSIVE_LOCKS_REQUIRED(mu_) {
    TF_RETURN_IF_ERROR(cinfo_.Init(ctx->resource_manager(), def()));
    GenomeIndexResource* genome_index;

    auto creator = [this, genome_location_](GenomeIndexResource** genome) {
        *genome = new GenomeIndexResource();
        (*genome)->init(genome_location_); 
        return Status::OK();
    };
