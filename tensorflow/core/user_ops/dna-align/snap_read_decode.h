...
class MutableSnapReadDecode {
  ...
  public:
    ...
    bool set_all_bases(const Tensor &bases);
    bool set_all_qualities(const Tensor &qualities);
    bool set_all_metadata(const Tensor &metadata);

  private:
    bool valid_vector(const Tensor &bases);
